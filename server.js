...
const STORAGE_DIR = path.resolve(process.env.APP_STORAGE_DIR || path.join(ROOT, 'storage'));
const STORE_FILE = path.join(DATA_DIR, 'store.json');
const API_PREFIX = '/api/v1';
const MAX_BODY_BYTES = 64 * 1024;
const encryptionKey = crypto.createHash('sha256').update(process.env.DATA_ENCRYPTION_KEY || 'development-only-key-change-me').digest();
const rateBuckets = new Map();
const stats = { startedAt: new Date().toISOString(), requests: 0, errors: 0, generated: 0 };
...
class JsonStore {
  constructor(file) { this.file = file; this.data = { searches: [], tasks: [], videoTasks: [], questions: [], attempts: [], mistakes: [], diagnostics: [] }; this.writeQueue = Promise.resolve(); }
  async init() {
    ...
    try { this.data = { ...this.data, ...JSON.parse(await fs.readFile(this.file, 'utf8')) }; }
    catch (error) { if (error.code !== 'ENOENT') throw error; await this.persist(); }
  }
  persist() {
    this.writeQueue = this.writeQueue.then(() => fs.writeFile(this.file, JSON.stringify(this.data, null, 2), 'utf8'));
    return this.writeQueue;
  }
  async add(collection, record) { this.data[collection].push(record); await this.persist(); return record; }
  find(collection, id) { return this.data[collection].find((entry) => entry.id === id); }
  async update(collection, id, update) { const item = this.find(collection, id); if (!item) return null; Object.assign(item, update, { updatedAt: new Date().toISOString() }); await this.persist(); return item; }
}
...
function cleanText(value, name = 'text') {
  if (typeof value !== 'string') throw new ApiError(400, 'INVALID_INPUT', `${name} 必须是字符串`);
  const normalized = value.normalize('NFC').replace(/\r\n?/g, '\n').replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, '').replace(/[\t ]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim();
  if (!normalized) throw new ApiError(400, 'EMPTY_TEXT', `${name} 不能为空`);
  if (normalized.length > 4000) throw new ApiError(413, 'TEXT_TOO_LONG', `${name} 最多 4000 个字符`);
...
function safeFileName(prefix, ext) { return `${prefix}-${Date.now()}-${crypto.randomUUID()}${ext}`; }
function publicAsset(file) { return `/storage/${file.replaceAll(path.sep, '/')}`; }
...
function encryptSensitive(value) {
  ...
  if (typeof value !== 'string' || value.length > 256) throw new ApiError(400, 'INVALID_INPUT', 'userRef 必须是不超过 256 字符的字符串');
  const iv = crypto.randomBytes(12); const cipher = crypto.createCipheriv('aes-256-gcm', encryptionKey, iv);
  const data = Buffer.concat([cipher.update(value.normalize('NFC'), 'utf8'), cipher.final()]);
  return `v1.${iv.toString('base64url')}.${cipher.getAuthTag().toString('base64url')}.${data.toString('base64url')}`;
}
function errorPayload(error, id) { return { error: { code: error.code || 'INTERNAL_ERROR', message: error.expose ? error.message : (error.status ? error.message : '服务器内部错误'), details: error.details }, requestId: id }; }
function send(res, status, payload, requestIdValue) {
  res.writeHead(status, { 'content-type': 'application/json; charset=utf-8', 'x-request-id': requestIdValue, 'cache-control': 'no-store' });
  res.end(JSON.stringify(payload));
}
async function body(req) {
  const chunks = []; let size = 0;
  for await (const chunk of req) { size += chunk.length; if (size > MAX_BODY_BYTES) throw new ApiError(413, 'PAYLOAD_TOO_LARGE', '请求体不能超过 64KB'); chunks.push(chunk); }
  if (!chunks.length) return {};
  try { return JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { throw new ApiError(400, 'INVALID_JSON', '请求体必须是有效 JSON'); }
}
function getClientIp(req) { return (req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown').toString().split(',')[0].trim(); }
function allowRequest(req) {
  const ip = getClientIp(req); const stamp = Date.now(); const bucket = rateBuckets.get(ip) || [];
  const recent = bucket.filter((time) => time > stamp - 60_000);
  if (recent.length >= 60) return false;
  recent.push(stamp); rateBuckets.set(ip, recent); return true;
}
function auth(req) {
  if (!process.env.API_KEY) return;
  const token = req.headers.authorization?.replace(/^Bearer\s+/i, '') || req.headers['x-api-key'];
  const expected = Buffer.from(process.env.API_KEY); const actual = Buffer.from(token || '');
...
function getTopic(text) {
  const entries = [
    [/快速排序|排序|sort/i, ['快速排序', '算法']], [/TCP|握手|网络|HTTP|HTTPS/i, ['TCP 协议基础', '计算机网络']],
    [/进程|线程|死锁|操作系统/i, ['进程与线程', '操作系统']], [/二叉树|树|遍历/i, ['二叉树遍历', '数据结构']],
    [/数据库|SQL|事务/i, ['数据库事务', '数据库']], [/二分|查找|search/i, ['二分查找', '算法']]
  ];
  const match = entries.find(([pattern]) => pattern.test(text));
  return match ? { title: match[1][0], subject: match[1][1] } : { title: text.slice(0, 40), subject: '综合学习' };
}
function buildLessonPlan(questionText, topic) {
  const stepLibrary = {
    '快速排序': ['确定基准值 pivot，并说明它用于划分待排序区间。', '从两端扫描，把小于基准值的元素移到左侧、大于基准值的元素移到右侧。', '基准值归位后，对左右两个子区间递归执行相同过程。'],
    'TCP 协议基础': ['先明确通信双方：客户端与服务器，以及建立可靠连接的目标。', '按时间顺序标出 SYN、SYN+ACK、ACK 三次报文，并说明每一次确认的作用。', '结合时序图核对连接建立后双方均具备收发能力。'],
    '进程与线程': ['先区分资源拥有者与执行单位，建立进程、线程两个概念框。', '从地址空间、调度开销、通信方式三个维度逐项比较。', '结合并发程序场景总结何时选择进程或线程。'],
    '二叉树遍历': ['标出根节点、左子树和右子树，建立树形结构图。', '按题目要求确定访问顺序，并在每次访问节点时做编号标记。', '将访问序列逐项写出，再检查每个节点是否恰好访问一次。'],
    '数据库事务': ['识别操作是否需要作为一个整体完成，确定事务边界。', '用 ACID 四个维度分析原子性、一致性、隔离性和持久性。', '结合提交、回滚与并发访问验证最终结果。'],
    '二分查找': ['确认待查数据已经有序，标出左右边界和中间位置。', '比较目标值与中间值，排除不可能的一半区间。', '重复更新边界，直到找到目标或区间为空。']
  };
  ...
  const scenes = [
    { index: 1, name: '完整题目展示与目标定位', visual: '全屏展示完整题干；高亮已知条件、问题和关键词', narration: `现在来看完整题目：${questionText}。先不要急于计算，先圈出题目给出的条件，并明确本题最终需要求什么。` },
    { index: 2, name: '解题思路与核心知识点', visual: `知识点卡片：${topic.title}；用关系图连接条件与目标`, narration: `这道题考查的是${topic.title}。我们的总体思路是先建立正确的分析框架，再按步骤逐一推导。` },
    ...steps.map((step, index) => ({ index: index + 3, name: `关键步骤 ${index + 1}`, visual: index === 1 ? '逐步标注、箭头与关系/流程图分析' : '公式、表格或结构图上的动态高亮', narration: `第 ${index + 1} 步，${step}` })),
    { index: steps.length + 3, name: '结论检验与方法总结', visual: '高亮最终结论；用检查清单回顾关键步骤', narration: `最后回顾：先读懂题意，再抓住${topic.title}的核心方法，按照刚才的步骤完成推导，并用题目条件检查结论是否合理。` }
  ];
  return {
    questionText,
    topic: topic.title,
...
function generateQuestions(text, config = {}) {
  const difficulty = enumValue(config.difficulty, ['easy', 'medium', 'hard'], 'medium', 'difficulty');
  const requested = Array.isArray(config.types) && config.types.length ? config.types : ['choice', 'true_false', 'fill_blank'];
  const types = requested.map((type) => enumValue(type, ['choice', 'true_false', 'fill_blank'], null, '题型'));
  const count = Math.min(Math.max(Number(config.count || 3), 1), 10);
  const topic = getTopic(text); const idBase = crypto.randomUUID();
  const facts = {
    '快速排序': ['分治', '基准值 pivot', 'O(n log n)'], 'TCP 协议基础': ['可靠传输', '三次握手', '确认机制'],
    '进程与线程': ['资源分配', 'CPU 调度', '并发执行'], '二叉树遍历': ['前序遍历', '中序遍历', '后序遍历'],
    '数据库事务': ['原子性', '一致性', '隔离性'], '二分查找': ['有序', '中间元素', 'O(log n)']
  };
...
function normalizeTopic(raw, fallbackText) {
  if (raw && typeof raw === 'object' && raw.title) return { title: String(raw.title).slice(0, 60), subject: String(raw.subject || '综合学习').slice(0, 40) };
  const parsed = getTopic(String(raw || fallbackText || '综合学习'));
  return { title: parsed.title, subject: parsed.subject };
}
const MISTAKE_CAUSES = ['concept', 'confusion', 'method', 'misread', 'careless', 'calc', 'unknown'];
function mistakeCauseLabel(cause) {
  return ({ concept: '概念不清', confusion: '概念混淆', careless: '粗心失误', method: '方法不当', misread: '审题失误', calc: '计算失误', unknown: '暂无法判断' })[cause] || '暂无法判断';
}
...
async function analyzeMistakeWithLLM(record) {
  ...
  if (!apiKey) return { cause: 'unknown', causeText: '未配置 QWEN_API_KEY，无法进行 AI 归因', suggestion: `建议重新学习「${record.topic.title}」相关知识点后重做本题。` };
...
{"cause":"concept|confusion|method|misread|careless|calc|unknown","causeText":"一句话点明具体错误原因（不超过 30 字）","suggestion":"具体可操作的学习建议，引用对应知识点或学习资源"}`;
  ...
  log('mistake.analyze.request', { mistakeId: record.id, topic: record.topic.title });
  try {
    const res = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model, messages: [{ role: 'system', content: system }, { role: 'user', content: prompt }], max_tokens: 512 }),
      signal: AbortSignal.timeout(60_000),
    });
    if (!res.ok) throw new ApiError(502, 'PROVIDER_ERROR', `Qwen 返回 ${res.status}`);
    ...
    log('mistake.analyze.success', { mistakeId: record.id, cause, ms: Date.now() - startedAt });
    return result;
  } catch (e) {
    log('mistake.analyze.failed', { mistakeId: record.id, code: e.code || 'UNKNOWN', message: e.message, ms: Date.now() - startedAt });
    return { cause: 'unknown', causeText: 'AI 归因暂时不可用', suggestion: `建议先复习「${record.topic.title}」相关知识点，再重做本题。` };
  }
}
// 学情诊断聚合（本地统计 + 掌握度分级）
function buildDiagnostics(clientId, from, to) {
  ...
  const list = store.data.attempts.filter((a) => a.clientId === clientId && inRange(a.createdAt));
  const total = list.length;
  const correct = list.filter((a) => a.correct).length;
  const byTopic = new Map();
  for (const a of list) {
    const key = a.topic?.title || '未分类';
    const entry = byTopic.get(key) || { topic: key, subject: a.topic?.subject || '综合学习', attempts: 0, correct: 0 };
    entry.attempts++;
    if (a.correct) entry.correct++;
    byTopic.set(key, entry);
  }
  const topics = [...byTopic.values()].map((t) => ({ ...t, correctRate: t.attempts ? t.correct / t.attempts : 0, masteryLevel: masteryLevel(t.attempts, t.correct) })).sort((x, y) => x.correctRate - y.correctRate);
  const mistakes = store.data.mistakes.filter((m) => m.clientId === clientId && inRange(m.createdAt));
  ...
  const trend = [...trendMap.values()].map((t) => ({ ...t, accuracy: t.attempts ? t.correct / t.attempts : 0 })).sort((x, y) => x.date.localeCompare(y.date));
  return {
    summary: {
      totalAttempts: total,
      ...
      reviewRate: mistakes.length ? Math.round((reviewed / mistakes.length) * 100) / 100 : 0,
      trend,
    },
  };
}
...
async function callProvider(url, apiKey, payload) {
  const response = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json', ...(apiKey ? { authorization: `Bearer ${apiKey}` } : {}) }, body: JSON.stringify(payload), signal: AbortSignal.timeout(90_000) });
  if (!response.ok) throw new ApiError(502, 'PROVIDER_ERROR', `第三方生成服务返回 ${response.status}`);
  const result = await response.json(); const assetUrl = result.url || result.data?.url || result.output?.url;
  if (!assetUrl || typeof assetUrl !== 'string') throw new ApiError(502, 'PROVIDER_INVALID_RESPONSE', '第三方服务未返回媒体 URL');
  return assetUrl;
}
async function writeSilentWav(text, speed) {
  const seconds = Math.max(1, Math.min(10, Math.ceil(text.length / (15 * speed)))); const rate = 8000; const bytes = seconds * rate * 2;
  ...
  const file = path.join('audio', safeFileName('tts', '.wav')); await fs.writeFile(path.join(STORAGE_DIR, file), Buffer.concat([header, Buffer.alloc(bytes)]));
  return { url: publicAsset(file), mimeType: 'audio/wav', provider: 'local-development', durationSeconds: seconds };
}
// ── 豆包 Seedance 视频生成 ──
async function createDoubaoTask(query) {
  if (!process.env.ARK_API_KEY) throw new ApiError(503, 'SERVICE_UNAVAILABLE', '未配置 ARK_API_KEY，请在 .env 中设置');
  ...
  if (!res.ok) {
    ...
    throw new ApiError(502, 'PROVIDER_ERROR', `豆包返回 ${res.status}: ${err.error?.message || '未知错误'}`);
  }
  const data = await res.json();
...
async function getDoubaoTaskStatus(taskId) {
  ...
  return { status: data.status, video_url: data.status === 'succeeded' ? data.content?.video_url : null, error: data.extra?.error, updated_at: data.updated_at };
}
...
async function generateSlides(query, persona) {
  ...
  const tryProvider = async (provider) => {
    ...
    if (!apiKey) throw new ApiError(503, 'SERVICE_UNAVAILABLE', `未配置 ${isQwen ? 'QWEN' : 'ARK'}_API_KEY`);
    const model = isQwen
...
11. 代码是否生成取决于知识点本身的性质：若知识点与编程实现相关（算法、数据结构、编程语言、代码语法、排序、查找、遍历等），则算法/公式页与实例演示页应包含带注释的 code 代码块（配 lang 字段），并与公式、实例一一对应；若知识点是纯理论或概念类（如 TCP 三次握手、进程与线程、数据库事务、网络协议等），则不强求代码，改用流程图、对比表、状态图表达核心逻辑`;
  ...
  try {
    ...
    if (useQwen) {
      ...
      try { return await tryProvider('ark'); } catch (e2) {
        throw new ApiError(502, 'PROVIDER_ERROR', `所有 LLM 调用失败: ${e2.message}${e.message ? `（Qwen: ${e.message}）` : ''}`);
      }
    }
    throw e;
  }
...
async function generateQuizViaLLM(query, variantsOf) {
  ...
  const isVariant = Boolean(variantsOf && (variantsOf.question || variantsOf.topic));
...
4. 题干表述严谨、无歧义，适合教学测评`;
  ...
  const res = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
    body: JSON.stringify({ model, messages: [{ role: 'system', content: system }, { role: 'user', content: prompt }], max_tokens: 2048 }),
    signal: AbortSignal.timeout(90_000),
  });
  const httpMs = Date.now() - startedAt;
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    log('quiz.llm.http_error', { query, status: res.status, ms: httpMs, message: err.error?.message || '未知错误' });
  ...
  if (!raw) {
    log('quiz.llm.empty_content', { query, ms: Date.now() - startedAt, finishReason: data.choices?.[0]?.finish_reason, usage });
  ...
  if (!Array.isArray(json.questions) || !json.questions.length) {
    log('quiz.llm.invalid_structure', { query, ms: Date.now() - startedAt, type: Array.isArray(json) ? 'array' : typeof json, keys: json && typeof json === 'object' ? Object.keys(json).slice(0, 10) : null });
    throw new Error('题目内容为空');
  }
  const questions = json.questions.slice(0, count).map((item) => {
    const opts = (Array.isArray(item.options) && item.options.length >= 2) ? item.options.slice(0, 4) : ['A', 'B', 'C', 'D'];
    let ans = Number(item.answer);
    if (!Number.isInteger(ans) || ans < 0 || ans >= opts.length) ans = 0;
    return { q: String(item.q || ''), opts, ans, exp: String(item.explanation || '暂无解析') };
  });
  log('quiz.llm.success', { query, ms: Date.now() - startedAt, rawLength: raw.length, questions: questions.length, answers: questions.map((q) => q.ans), usage });
  return questions;
}

// ── 变式题本地回退：LLM 失败时基于原题做数值扰动与选项重排 ──
function generateVariantsFallback(original) {
  const opts = Array.isArray(original.options) && original.options.length ? original.options : ['A', 'B', 'C', 'D'];
  const ansText = typeof original.answer === 'number' && opts[original.answer] ? opts[original.answer] : '';
  const questions = [];
  for (let k = 0; k < 2; k++) {
...
function slidesToMarkdown(slides, fullText, query) {
  const diagramNames = { line_chart: '折线图', bar_chart: '柱状图', complexity_curve: '复杂度曲线', array: '数组状态图', flow: '流程图', tree: '树形结构', compare: '对比表' };
  ...
  const lines = [`# ${query} 学习笔记`, '', `> 生成时间：${now()}`, '', '---', ''];
  slides.forEach((s, i) => {
    lines.push(`## ${i + 1}. ${s.title}`, '');
    if (s.subtitle) lines.push(`> ${s.subtitle}`, '');
    if (s.text) lines.push(s.text, '');
    if (s.left) {
      String(s.left).split('\n').filter(Boolean).forEach((it) => lines.push(`- ${it.replace(/^[-•]\s*/, '')}`));
      lines.push('');
    }
    (s.visuals || []).forEach((v) => {
      const d = String(v.data || '');
      if (v.type === 'formula') { lines.push('$$', d, '$$', ''); }
      else if (v.type === 'code') { lines.push('```' + (v.lang || ''), d, '```', ''); }
      else if (v.type === 'table' || v.type === 'compare') { lines.push(tableToMd(d), ''); }
      else if (v.type === 'list') { d.split('\n').filter(Boolean).forEach((it) => lines.push(`- ${it.replace(/^[-•]\s*/, '')}`)); lines.push(''); }
      else if (v.type === 'highlight' || v.type === 'quote') { lines.push(`> **${v.type === 'highlight' ? '要点' : '提示'}：** ${d}`, ''); }
      else if (v.type === 'badge') { lines.push(`**关键词：** ${d}`, ''); }
      else { lines.push(`- ${d}`, ''); }
...
async function chatSolveLLM(query, history) {
  ...
  const model = process.env.QWEN_CHAT_MODEL || 'qwen-max';
  const system = '你是"栈知映"的 AI 解题导师，一名资深计算机科学教育专家，擅长把题目拆解成清晰、循序渐进的小步骤讲解。\n'
    + '输出规范：\n'
  ...
  const safeHistory = (Array.isArray(history) ? history : []).slice(-8).map((h) => {
    const role = String(h.role) === 'assistant' ? 'assistant' : 'user';
    let content;
    try { content = cleanText(String(h.content ?? ''), 'history'); } catch { content = String(h.content ?? '').slice(0, 4000); }
    return { role, content };
  }).filter((h) => h.content);
  const messages = [{ role: 'system', content: system }, ...safeHistory, { role: 'user', content: query }];
  log('chat.llm.request', { query, turns: safeHistory.length, model, max_tokens: 2048 });
  const res = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    log('chat.llm.http_error', { status: res.status, message: err.error?.message || '未知错误' });
    throw new ApiError(502, 'PROVIDER_ERROR', `Qwen 返回 ${res.status}: ${err.error?.message || '未知错误'}`);
  }
  return res;
}
...
function buildFallbackChatReply(query, history) {
  ...
  plan.solutionSteps.forEach((step, index) => { lines.push(`${index + 1}. ${step}`); });
...
async function qwenTtsBuffer(text) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new ApiError(502, 'PROVIDER_ERROR', `Qwen TTS 返回 ${res.status}: ${err.message || err.error?.message || JSON.stringify(err).slice(0, 200)}`);
  }
  const data = await res.json();
  // 非流式输出：{ output: { audio: { url: '...', id: '...', expires_at: ... } } }
  const audioUrl = data.output?.audio?.url || data.output?.url || data.url || data.data?.url || data.output?.audio_url;
  if (!audioUrl || typeof audioUrl !== 'string') {
    throw new ApiError(502, 'PROVIDER_ERROR', `Qwen TTS 未返回音频 URL: ${JSON.stringify(data).slice(0, 300)}`);
  }
  const audio = await fetch(audioUrl, { signal: AbortSignal.timeout(60_000) });
  if (!audio.ok) throw new ApiError(502, 'PROVIDER_ERROR', `下载 Qwen TTS 音频失败: ${audio.status}`);
...
async function createTTS(script) {
  ...
  for (const chunk of chunks) {
    try {
      ...
      log('qwen.tts.chunk_failed', { chunkLength: chunk.length, message: e.message });
      // 单块失败不影响已完成块，继续合成剩余旁白
    }
  }
  if (!buffers.length) {
    throw new ApiError(502, 'PROVIDER_ERROR', 'TTS 合成失败，请检查 QWEN_API_KEY 配置');
  }
...
async function createPoster(text, topic, lessonPlan) {
  ...
  const file = path.join('poster', safeFileName('lesson', '.svg')); const title = escape(topic.title); const summary = escape(text.slice(0, 48)); const step = escape(lessonPlan.solutionSteps[0]);
  ...
  await fs.writeFile(path.join(STORAGE_DIR, file), svg, 'utf8'); return { url: publicAsset(file), mimeType: 'image/svg+xml', provider: 'local-development' };
}
async function makeAudio(text, voice) { if (process.env.TTS_PROVIDER_URL) return { url: await callProvider(process.env.TTS_PROVIDER_URL, process.env.TTS_API_KEY, { text, voice }), mimeType: 'audio/mpeg', provider: 'third-party' }; return writeSilentWav(text, voice.speed); }
async function makeVideo(text, topic, lessonPlan, audio) {
  if (process.env.VIDEO_PROVIDER_URL) {
    const videoBrief = {
      taskType: 'educational-problem-explanation', aspectRatio: '16:9', language: 'zh-CN',
      requirements: ['完整展示题目内容', '逐步讲解解题思路与依据', '为题干、关键步骤和图表使用同步视觉标注', '音频旁白与每个分镜严格同步', '结尾给出结论与方法总结'],
      question: text,
      lessonPlan,
      voiceover: {
        audioUrl: audio.url,
        voiceType: audio.voiceType,
        syncInstruction: '将每个 scene.narration 与对应 scene.visual 同步；旁白开始时展示该分镜，旁白结束后再进入下一分镜。'
      }
    };
    return { url: await callProvider(process.env.VIDEO_PROVIDER_URL, process.env.VIDEO_API_KEY, videoBrief), mimeType: 'video/mp4', provider: 'third-party' };
  }
  return createPoster(text, topic, lessonPlan);
}

async function runTask(taskId) {
  const task = store.find('tasks', taskId); if (!task) return;
  try {
    await store.update('tasks', taskId, { status: 'processing', progress: 15 });
    const { text, voice, quiz } = task.request; const generated = generateQuestions(text, quiz); const lessonPlan = buildLessonPlan(text, generated.topic);
    for (const question of generated.questions) await store.add('questions', { ...question, taskId, createdAt: now() });
    ...
    const scenes = lessonPlan.scenes.map((scene) => ({ ...scene, durationSeconds: 14 }));
    const result = { topic: generated.topic, lessonPlan, audio, video, poster: video.mimeType === 'image/svg+xml' ? video : undefined, scenes, questions: generated.questions };
    await store.update('tasks', taskId, { status: 'completed', progress: 100, result }); stats.generated += 1; log('task.completed', { taskId });
  } catch (error) {
    stats.errors += 1; await store.update('tasks', taskId, { status: 'failed', progress: 100, error: { code: error.code || 'GENERATION_FAILED', message: error.message } }); log('task.failed', { taskId, code: error.code || 'GENERATION_FAILED' });
  }
}
function publicTask(task) { return { id: task.id, status: task.status, progress: task.progress, statusUrl: `${API_PREFIX}/tasks/${task.id}`, resultUrl: `${API_PREFIX}/tasks/${task.id}/result`, ...(task.error ? { error: task.error } : {}) }; }
async function createGeneration(payload) {
  const text = cleanText(payload.text ?? payload.query);
  const voiceInput = payload.voice || {}; const voice = { type: enumValue(voiceInput.type, ['female', 'male', 'child', 'neutral'], 'neutral', 'voice.type'), speed: Number(voiceInput.speed ?? 1) };
  if (!Number.isFinite(voice.speed) || voice.speed < 0.5 || voice.speed > 2) throw new ApiError(400, 'INVALID_INPUT', 'voice.speed 必须在 0.5 到 2 之间');
  const quiz = payload.quiz || {}; const generated = generateQuestions(text, quiz); // validate quiz before queuing
  const search = await store.add('searches', { id: requestId(), text, source: String(payload.source || 'api').slice(0, 32), encryptedUserRef: encryptSensitive(payload.userRef), createdAt: now() });
  const task = await store.add('tasks', { id: requestId(), searchId: search.id, status: 'queued', progress: 0, request: { text, voice, quiz: { ...quiz, types: generated.questions.map((item) => item.type) } }, createdAt: now(), updatedAt: now() });
  setImmediate(() => runTask(task.id)); return { search, task };
}

async function api(req, res, url, id) {
  const route = url.pathname.slice(API_PREFIX.length); if (!allowRequest(req)) throw new ApiError(429, 'RATE_LIMITED', '请求过于频繁，请稍后再试'); auth(req);
  if (route === '/health' && req.method === 'GET') return send(res, 200, { status: 'ok', service: 'ai-learning-studio', time: now() }, id);
  if (route === '/metrics' && req.method === 'GET') return send(res, 200, { ...stats, uptimeSeconds: Math.floor(process.uptime()), tasks: { queued: store.data.tasks.filter((t) => t.status === 'queued').length, processing: store.data.tasks.filter((t) => t.status === 'processing').length, failed: store.data.tasks.filter((t) => t.status === 'failed').length } }, id);
  if (route === '/search' && req.method === 'POST') { const input = await body(req); const text = cleanText(input.query ?? input.text, 'query'); const search = await store.add('searches', { id: requestId(), text, source: String(input.source || 'search-page').slice(0, 32), encryptedUserRef: encryptSensitive(input.userRef), createdAt: now() }); return send(res, 201, { data: { id: search.id, query: text, normalized: true, createdAt: search.createdAt }, requestId: id }, id); }
  // ── 幻灯片视频生成（异步） ──
  if (route === '/video/generate' && req.method === 'POST') {
    ...
    setImmediate(async () => {
      ...
      try {
        ...
        await store.update('videoTasks', taskId, { audioUrl, progress: 100, status: 'slides_ready' });
      } catch (e) {
        log('video.generate.failed', { taskId, message: e.message });
        if (slides.length) {
          // 讲稿已生成但语音合成失败：仍可播放无声幻灯片
          await store.update('videoTasks', taskId, { status: 'slides_ready', slides, fullText, audioUrl, progress: 100 });
        } else {
          // 讲稿生成失败：标记失败并保留错误信息，供前端展示
          await store.update('videoTasks', taskId, { status: 'failed', slides, fullText, audioUrl, progress: 100, error: { code: e.code || 'GENERATION_FAILED', message: e.message } });
        }
      }
    });
    return send(res, 201, { data: { task_id: taskId, audio_ready: false }, requestId: id }, id);
  }
  ...
  if (route === '/video/export' && req.method === 'GET') {
    ...
    const videoTask = store.find('videoTasks', taskId);
    if (!videoTask) throw new ApiError(404, 'TASK_NOT_FOUND', '未找到该任务');
    if (!videoTask.slides?.length) throw new ApiError(404, 'NO_CONTENT', '该任务暂无讲解内容');
    const query = videoTask.query || '学习笔记';
    const markdown = slidesToMarkdown(videoTask.slides, videoTask.fullText || '', query);
    return send(res, 200, { data: { filename: `学习笔记-${String(query).slice(0, 30)}.md`, query, markdown }, requestId: id }, id);
  }
  ...
  if (route === '/video/quiz' && req.method === 'POST') {
    ...
    try {
      ...
      log('quiz.request_invalid', { requestId: id, code: e.code, message: e.message });
      throw e;
    }
    log('quiz.request', { requestId: id, query: text, bodyKeys: Object.keys(input) });
    // 举一反三：可选参数 variants_of，存在时基于原题生成变式题
    const variantsOf = (input.variants_of && typeof input.variants_of === 'object')
      ? {
          topic: String(input.variants_of.topic || '').slice(0, 60),
          question: String(input.variants_of.question || '').slice(0, 500),
          options: Array.isArray(input.variants_of.options) ? input.variants_of.options.map((o) => String(o).slice(0, 200)) : [],
          answer: input.variants_of.answer,
          explanation: String(input.variants_of.explanation || '').slice(0, 1000),
        }
      : null;
    let questions, source = 'llm';
    try {
      questions = await generateQuizViaLLM(text, variantsOf);
    } catch (e) {
      source = 'fallback_rules';
      log('quiz.llm.failed', { requestId: id, query: text, variants: Boolean(variantsOf), code: e.code || 'UNKNOWN', message: e.message, ms: Date.now() - startedAt });
      try {
        if (variantsOf) {
        ...
        log('quiz.fallback_failed', { requestId: id, query: text, message: e2.message, ms: Date.now() - startedAt });
        throw e2;
      }
    }
    log('quiz.done', { requestId: id, query: text, source, questions: questions.length, answers: questions.map((q) => q.ans), ms: Date.now() - startedAt });
  ...
  if (route === '/chat' && req.method === 'POST') {
    ...
    log('chat.request', { requestId: id, query: question, turns: history.length });
    res.writeHead(200, { 'content-type': 'text/event-stream; charset=utf-8', 'cache-control': 'no-store', connection: 'keep-alive', 'x-request-id': id });
    ...
    try {
      ...
      } catch (e) { log('chat.stream.read_error', { requestId: id, message: e.message }); }
      if (!content) throw new Error('Qwen 未返回任何内容');
      log('chat.llm.success', { requestId: id, query: question, ms: Date.now() - startedAt, chars: content.length, usage });
      emit({ type: 'done', content, source: 'llm' });
    } catch (e) {
      log('chat.llm.failed', { requestId: id, query: question, code: e.code || 'UNKNOWN', message: e.message, ms: Date.now() - startedAt });
      emit({ type: 'done', content: buildFallbackChatReply(question, history), source: 'fallback_rules' });
    }
  ...
  if (route === '/attempts' && req.method === 'POST') {
    ...
    const type = enumValue(input.type, ['choice', 'true_false', 'fill_blank'], 'choice', 'type');
    const questionId = typeof input.questionId === 'string' ? input.questionId.slice(0, 64) : null;
    ...
    else throw new ApiError(422, 'CANNOT_GRADE', '无法判分：缺少有效的 questionId 或 correct 字段');
    const attempt = await store.add('attempts', {
      id: requestId(), clientId, questionId,
      taskId: typeof input.taskId === 'string' ? input.taskId.slice(0, 64) : null,
      topic, type, answer, correct, createdAt: now(),
    });
    let mistake = null;
    if (!correct) {
      const correctAnswer = question ? String(question.correctAnswer || '') : String(input.correctAnswer ?? '');
      const questionText = question ? String(question.content || '') : String(input.question ?? '');
      mistake = await store.add('mistakes', {
        id: requestId(), attemptId: attempt.id, clientId, questionId, taskId: attempt.taskId, topic, type,
        question: questionText, userAnswer: answer, correctAnswer,
        options: Array.isArray(input.options) ? input.options.slice(0, 8).map((o) => String(o).slice(0, 200)) : (question && Array.isArray(question.options) ? question.options : []),
        cause: null, causeText: null, suggestion: null,
        reviewed: false, reviewedAt: null, reviewCorrect: null, createdAt: now(),
      });
    }
    log('attempt.recorded', { requestId: id, clientId, topic: topic.title, type, correct, mistake: mistake ? mistake.id : null });
    return send(res, 200, { data: { attemptId: attempt.id, correct, mistakeId: mistake ? mistake.id : null }, requestId: id }, id);
  }
  // ── 学情：错题本列表 ──
  if (route === '/mistakes' && req.method === 'GET') {
    ...
    const items = list.map((m) => {
      ...
      if ((!Array.isArray(options) || !options.length) && m.questionId) {
        ...
        if (q && Array.isArray(q.options) && q.options.length) options = q.options;
      }
      return { ...m, options: Array.isArray(options) ? options : [], causeLabel: mistakeCauseLabel(m.cause), pending: !m.cause };
    });
    return send(res, 200, { data: { total: items.length, items }, requestId: id }, id);
  }
  // ── 学情：错题 AI 归因 ──
  ...
  if (mistakeAnalyzeMatch && req.method === 'POST') {
    ...
    return send(res, 200, { data: { id: mistake.id, ...result, causeLabel: mistakeCauseLabel(result.cause) }, requestId: id }, id);
  }
  // ── 学情：错题复习标记 ──
  ...
  if (mistakeReviewMatch && req.method === 'POST') {
    ...
    return send(res, 200, { data: { id: updated.id, reviewed: true, reviewCorrect: updated.reviewCorrect }, requestId: id }, id);
  }
  // ── 学情：学情诊断报告 ──
  if (route === '/diagnostics' && req.method === 'GET') {
    const clientId = getClientId(req);
    const fromRaw = url.searchParams.get('from'); const toRaw = url.searchParams.get('to');
    const from = fromRaw ? new Date(fromRaw).getTime() : null; const to = toRaw ? new Date(toRaw).getTime() : null;
    const startedAt = Date.now();
    ...
    try {
      ...
      if (weak.length && process.env.QWEN_API_KEY) {
        const system = '你是学情分析专家，请基于学生的薄弱知识点列表给出分析与建议。只输出合法 JSON。';
        ...
        const res = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
          method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.QWEN_API_KEY}` },
          body: JSON.stringify({ model: process.env.QWEN_CHAT_MODEL || 'qwen-max', messages: [{ role: 'system', content: system }, { role: 'user', content: prompt }], max_tokens: 1024 }),
          signal: AbortSignal.timeout(60_000),
        });
        if (res.ok) {
          const data = await res.json();
          ...
          if (Array.isArray(json?.weakTopics)) {
            report.summary.weakTopics = weak.map((w) => {
              ...
              return found ? { ...w, reason: String(found.reason || '').slice(0, 200), suggestion: String(found.suggestion || '').slice(0, 300) } : w;
            });
          }
        }
      }
    } catch (e) { log('diagnostics.ai.failed', { requestId: id, message: e.message }); }
    await store.add('diagnostics', { id: requestId(), clientId, ...report, createdAt: now() });
    log('diagnostics.generated', { requestId: id, clientId, totalAttempts: report.summary.totalAttempts, weak: report.summary.weakTopics.length, ms: Date.now() - startedAt });
    return send(res, 200, { data: report, requestId: id }, id);
  }
  if (route === '/generations' && req.method === 'POST') { const created = await createGeneration(await body(req)); return send(res, 202, { data: publicTask(created.task), requestId: id }, id); }
  const taskMatch = route.match(/^\/tasks\/([\w-]+)(\/result)?$/);
  if (taskMatch && req.method === 'GET') { const task = store.find('tasks', taskMatch[1]); if (!task) throw new ApiError(404, 'TASK_NOT_FOUND', '未找到该任务'); if (taskMatch[2]) { if (task.status !== 'completed') throw new ApiError(409, 'TASK_NOT_READY', '任务尚未完成'); return send(res, 200, { data: task.result, requestId: id }, id); } return send(res, 200, { data: publicTask(task), requestId: id }, id); }
  ...
  if (route === '/bilibili/cover' && req.method === 'GET') {
    ...
    try {
      ...
      if (!remote.ok) throw new ApiError(502, 'PROVIDER_ERROR', `Bilibili 返回 ${remote.status}`);
      const data = await remote.json();
      const cover = data.data?.pic;
      const title = data.data?.title;
      if (!title) throw new ApiError(502, 'PROVIDER_ERROR', '未获取到视频标题');
      return send(res, 200, { data: { bvid, title, cover: cover || '' }, requestId: id }, id);
    } catch (e) {
      if (e instanceof ApiError) throw e;
      throw new ApiError(502, 'PROVIDER_ERROR', `获取视频信息失败: ${e.message}`);
    }
  }
  if (route === '/questions/generate' && req.method === 'POST') { const input = await body(req); const generated = generateQuestions(cleanText(input.text), input); for (const question of generated.questions) await store.add('questions', { ...question, createdAt: now() }); return send(res, 201, { data: generated, requestId: id }, id); }
  const gradeMatch = route.match(/^\/questions\/([\w-]+)\/grade$/);
  if (gradeMatch && req.method === 'POST') { const question = store.find('questions', gradeMatch[1]); if (!question) throw new ApiError(404, 'QUESTION_NOT_FOUND', '未找到该题目'); const input = await body(req); const correct = grade(question, input.answer); await store.add('attempts', { id: requestId(), questionId: question.id, answer: String(input.answer), correct, createdAt: now() }); return send(res, 200, { data: { questionId: question.id, correct, explanation: question.explanation, correctAnswer: question.correctAnswer }, requestId: id }, id); }
...
async function staticFile(req, res, url, id) {
  ...
  if (rangeHeader && stat.isFile()) {
    ...
    if (match) { start = match[1] === '' ? null : Number(match[1]); end = match[2] === '' ? null : Number(match[2]); }
    if (match && !(start === null && end === null)) {
      if (start === null) { start = Math.max(0, stat.size - end); end = stat.size - 1; }
      else { if (start >= stat.size) { res.writeHead(416, { 'content-range': `bytes */${stat.size}`, ...headers }); return res.end(); } if (end === null || end >= stat.size) end = stat.size - 1; }
      if (start > end) { res.writeHead(416, { 'content-range': `bytes */${stat.size}`, ...headers }); return res.end(); }
      const fd = await fs.open(target, 'r');
      const buffer = Buffer.alloc(end - start + 1);
      await fd.read(buffer, 0, buffer.length, start);
      await fd.close();
      res.writeHead(206, { 'content-range': `bytes ${start}-${end}/${stat.size}`, 'content-length': buffer.length, ...headers });
...
const server = http.createServer(async (req, res) => {
  const id = requestId(); stats.requests += 1; const started = Date.now();
  res.setHeader('access-control-allow-origin', process.env.CORS_ORIGIN || '*'); res.setHeader('access-control-allow-methods', 'GET,POST,OPTIONS'); res.setHeader('access-control-allow-headers', 'Content-Type, Authorization, X-API-Key'); res.setHeader('x-content-type-options', 'nosniff'); res.setHeader('x-frame-options', 'SAMEORIGIN'); res.setHeader('referrer-policy', 'strict-origin-when-cross-origin');
  try { if (req.method === 'OPTIONS') { res.writeHead(204); return res.end(); } const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`); if (url.pathname.startsWith(API_PREFIX)) await api(req, res, url, id); else if (req.method === 'GET') await staticFile(req, res, url, id); else throw new ApiError(405, 'METHOD_NOT_ALLOWED', '仅支持 GET、POST 和 OPTIONS'); log('request', { id, method: req.method, path: url.pathname, ms: Date.now() - started }); }
